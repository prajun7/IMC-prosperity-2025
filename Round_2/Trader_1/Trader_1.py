...
class Trader:
    ...
    # Product-specific parameters based on observed behaviors in the charts
    PRODUCT_PARAMS = {
        "Picnic Basket 1": {
            "alpha": 0.25,           # EMA weight (moderate due to mixed patterns)
            "spread_factor": 0.6,    # Moderate spread for stability
            "trend_factor": 0.4,     # Moderate trend following
            "mean_reversion": True,  # Apply mean reversion
            "volatility_scale": 1.2, # Slightly higher volatility scaling
            "min_spread": 2,         # Minimum spread to maintain
            "take_width": 3,         # Distance from mid price to take orders
            "aggressive_edge": 0.4,  # Moderate aggression
            "risk_aversion": 0.4,    # Moderate risk aversion
            "max_position_scale": 0.9, # Slightly reduced max position for safety
        },
        "Djembes": {
            "alpha": 0.35,           # Medium-high EMA weight due to volatility
            "spread_factor": 0.8,    # Higher spread for volatility
            "trend_factor": 0.7,     # Higher trend following (significant trends in charts)
            "mean_reversion": False, # No mean reversion - follows trends
            "volatility_scale": 1.7, # Higher volatility scaling
            "min_spread": 3,         # Higher minimum spread
            "take_width": 4,         # Distance from mid price to take orders
            "aggressive_edge": 0.6,  # More aggressive due to volatility
            "risk_aversion": 0.6,    # Medium-high risk aversion
            "max_position_scale": 0.8, # Reduced max position for safety
        },
        "Croissants": {
            "alpha": 0.3,            # Medium EMA weight
            "spread_factor": 0.7,    # Medium spread
            "trend_factor": 0.5,     # Balanced trend following
            "mean_reversion": True,  # Apply mean reversion
            "volatility_scale": 1.4, # Medium-high volatility scaling
            "min_spread": 2,         # Moderate minimum spread
            "take_width": 3,         # Distance from mid price to take orders
            "aggressive_edge": 0.5,  # Balanced aggression
            "risk_aversion": 0.5,    # Balanced risk aversion
            "max_position_scale": 0.9, # Slightly reduced max position
        },
        "Jams": {
            "alpha": 0.3,            # Medium EMA weight
            "spread_factor": 0.7,    # Medium spread
            "trend_factor": 0.6,     # Medium-high trend following
