...
class Trader:
    ...
    # *** INCREASED ALPHA FOR FASTER EMA REACTION / MORE AGGRESSION ***
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            # Avoid division by zero or unusual states if bid >= ask
            ...
            # --- Slightly More Aggressive Buy Logic ---
            # If our fair value estimate (EMA) is >= the best ask, consider buying
            # Changed > to >=
            ...
            # --- Slightly More Aggressive Sell Logic ---
            # If our fair value estimate (EMA) is <= the best bid, consider selling
            # Changed < to <=
...
# In the Buy Logic: if acceptable_price > best_ask changed to if acceptable_price >= best_ask
# In the Sell Logic: elif acceptable_price < best_bid changed to elif acceptable_price <= best_bid
# Added a check if best_bid >= best_ask: continue to prevent errors if the order book gets crossed or has zero spread, which could cause issues with the mid-price or logic.
# ALPHA changed from 0.3 to 0.6.